...
def test_example1_safe_read_flow(tmp_config, tmp_path):
    ...
    agent = CeoAgent("ceo", MockProvider(), RollingBuffer(tmp_config), tmp_config)
...
def test_subagent_depth_limit(tmp_config):
    q = queue.Queue()
    s = SubAgent("sub1", ["filesystem.read"], max_depth=1, parent_id="ceo", task_queue=q, config=tmp_config, depth=1)
...
def test_plan_validation_proof_required(tmp_config):
    class BadProvider(MockProvider):
        def complete(self, messages, tools):
            return {"content": '{"steps":[{"id":"x","action":"n","tool":"network.fetch","args":{},"requires":["network"],"proof_required":true}]}' }
